...
CREATE POLICY "documentos_select_own_or_admin" ON storage.objects
FOR SELECT TO authenticated
USING (bucket_id = 'documentos' AND ((storage.foldername(name))[1] = auth.uid()::text OR public.tiene_rol(auth.uid(), 'admin')));
...
CREATE POLICY "documentos_delete_own_or_admin" ON storage.objects
